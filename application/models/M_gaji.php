...
class M_gaji extends CI_Model { 
	...
	function get_datatables($where)
	{
		...
		if($_GET['length'] != -1)
		$this->db->select("SUM(CASE WHEN absen_bayar != '' THEN absen_upah ELSE 0 END) AS total, SUM(CASE WHEN absen_bayar = 'sudah' THEN absen_upah ELSE 0 END) AS lunas, SUM(CASE WHEN absen_bayar = 'belum' THEN absen_upah ELSE 0 END) AS sisa, karyawan_id AS id, karyawan_nama AS nama");
		$this->db->where($where);
